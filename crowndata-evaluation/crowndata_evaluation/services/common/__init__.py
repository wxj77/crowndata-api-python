...
def test_define_clusters_invalid_input():
    """
    Test case where input data has invalid shape (not 2D).
    """
    data = np.array([1, 2, 3])  # 1D array instead of 2D
    epsilon = 0.5

    # Ensure that an error is raised when the input is invalid
