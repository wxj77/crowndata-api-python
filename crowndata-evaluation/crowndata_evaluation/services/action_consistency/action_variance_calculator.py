...
class ActionVarianceCalculator:
    ...
    def calculate_action_variance(self, states: np.ndarray, actions: np.ndarray) -> Dict[int, float]:
        ...
        for cluster_id, cluster_points in clusters.items():
            cluster_indices = np.array([np.where((states == point).all(axis=1))[0][0] for point in cluster_points])
